        ...
        if (err != null) {
          ...
          return Room.findOne({
            ...
            if (err2 != null) {
              ...
              return User.find().where('_id')["in"](clients).populate('chars').exec(function(err3, users) {
                var c, u, _i, _len;
                if (err3 != null) {
                  ...
                  for (_i = 0, _len = users.length; _i < _len; _i++) {
                    u = users[_i];
                    if (!((u.chars[0] != null) && u.visible === true)) {
                      continue;
                    }
          ...
          return Char.findOne({
            name: req.data[0]
          }).populate('owner').exec(function(err4, char) {
            var _ref;
            if (err4 != null) {
              return req.io.emit('error', err4);
            } else if (char == null) {
              return req.io.emit('message', "There is no such person.");
            } else if (_ref = char.owner._id.toString(), __indexOf.call(clients, _ref) < 0) {
              return req.io.emit('message', "" + char.name + " is nowhere to be seen.");
            } else if (char.owner.visible === false) {
              return req.io.emit('message', "" + char.name + " is nowhere to be seen.");
            } else {
              return char.owner.populate('chars', function(err5, owner) {
                var msg;
                if (err5 != null) {
                  return req.io.emit('error', err5);
                } else if (owner.chars[owner.currentChar].name !== char.name) {
                  return req.io.emit('message', "" + char.name + " is nowhere to be seen.");
                } else {
